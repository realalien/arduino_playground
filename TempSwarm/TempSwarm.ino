...
char ssid[] = "District9";  //  your network SSID (name)
...
// NOTE: we are chasing one variable of the environment, adding 
//       multiple variables complicated the swarm logic. For the
//       simplicity, use only Temp
//float humidity; 
float temperature;


const int PACKET_SIZE = 10; // Light Update Packet
...
void setup()
{
  ...
  Serial.println("");
  Serial.println("--------------------------");
  Serial.println("TempSwarm");
  Serial.print("Version ");
  Serial.println(VERSIONNUMBER);
  Serial.println("--------------------------");

  Serial.println(F(" 10/10/2017"));
...
//  } else {
//    Serial.println("No DHT22 found ... check your connections");
//  }
  ...
  Serial.print("TempSwarm Instance: ");
...
//  greenColor = g;
  ...
  if (!cb) {
    ...
    if (packetBuffer[1] == TEMP_UPDATE_PACKET)
    {
      Serial.print("TEMP_UPDATE_PACKET received from TempSwarm #");
      ...
      if (packetBuffer[3] == 0)
        ...
        Serial.print(*(float*)packetBuffer[5] ); // TODO: read correctly
...
//      Serial.print(packetBuffer[11] * 256 + packetBuffer[12]);
      ...
      swarmTemp[setAndReturnMySwarmIndex(packetBuffer[2])] = *(float*)packetBuffer[5] ; //packetBuffer[5] * 256 + packetBuffer[6];
...
// REF. https://stackoverflow.com/questions/24420246/c-function-to-convert-float-to-byte-array
//typedef char byte;
 
byte* floatToByteArray(float f) {
  byte* ret =  (unsigned char*)malloc(4 * sizeof(byte));
...
unsigned long sendLightUpdatePacket(IPAddress & address)
{
  ...
  packetBuffer[1] = TEMP_UPDATE_PACKET;     // Packet Type
  ...
  byte* asBytes = floatToByteArray(temperature);
  packetBuffer[5] = asBytes[0];
  packetBuffer[6] = asBytes[1];
  packetBuffer[7] = asBytes[2];
  packetBuffer[8] = asBytes[3];
  packetBuffer[9] = 0x0F;  //End Byte
...
void broadcastARandomUpdatePacket()
{
  ...
  IPAddress sendSwarmAddress(192, 168, 1, sendToTempSwarm); // my Swarm Address
...
void checkAndSetIfMaster()
{
  ...
  {
#ifdef DEBUG
    Serial.print("swarmTemp[");
    Serial.print(i);
    Serial.print("] = ");
    Serial.print(swarmTemp[i]);
  ...
  for (i = 0; i < SWARMSIZE; i++)
  {
    ...
    if (swarmTemp[mySwarmID] >= swarmTemp[i])
    {
      // I might be master!
  ...
  if (masterState == true)
  {
    ...
    else
    {
      ...
      for (i = 0; i < SWARMSIZE; i++)
      {
        ...
        sprintf(swarmString, " %i,%i,%i,%i,%s,%i ", i, swarmState[i], swarmVersion[i], swarmTemp[i], stateString, swarmAddresses[i]);
